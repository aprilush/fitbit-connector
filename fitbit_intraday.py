    ...
    def get_intraday_time_series(self, resource_path, from_datetime, to_datetime, format='json'):
        ...
        if (to_datetime.date() == from_datetime.date()):
            ...
            while (next < to_datetime.date()):
                out = combine(out, self.get_day_time_series(resource_path, next, format))
                next = next + delta
            out = combine(out, self.get_time_interval_time_series(resource_path, to_datetime.date(), time(0, 0), to_datetime.time(), format))
        return out
